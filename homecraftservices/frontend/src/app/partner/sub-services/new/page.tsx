...
export default function PartnerNewSubServicePage() {
  ...
  useEffect(() => {
    // Load parent services created by the partner (parentId null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || 'Failed to create sub-service.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 border-b border-gold/15 pb-4">
        ...
        <div>
          <h1 className="font-serif text-2xl font-bold text-primary">Add New Sub-Service</h1>
          <p className="text-xs text-foreground/50">Submit a new sub-service under one of your parent offerings</p>
        </div>
      </div>
      ...
      {parentServices.length === 0 && (
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
          <p className="text-xs text-amber-700 font-bold leading-normal">
            You must create at least one parent service before you can add sub-services.
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-[#C3AB84]/20 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            <input required type="text" value={name} onChange={e => setName(e.target.value)}
              className={inp} placeholder="e.g. Split AC Filter Wash" />
          </div>
        ...
        <div>
          ...
          <textarea rows={3} value={description} onChange={e => setDescription(e.target.value)}
            className={inp} placeholder="Describe what this sub-service covers..." />
        </div>
        ...
        <div className="pt-4 flex justify-end gap-3">
          ...
          <button type="submit" disabled={saving || parentServices.length === 0} className="flex items-center gap-2 px-8 py-3 bg-[#0F3D30] text-white rounded-full font-semibold text-sm hover:bg-[#0F3D30]/90 disabled:opacity-60 transition-colors shadow-sm">
